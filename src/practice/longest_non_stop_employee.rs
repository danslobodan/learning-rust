...
fn longest_busy_time(working_slots: Vec<Vec<u8>>) -> u8 {
    let mut employee_longest_nonstop_work = Vec::new();

    for i in working_slots {
        let longest = longest_period(i);
        employee_longest_nonstop_work.push(longest);
    }
    ...
    for i in 0..employee_longest_nonstop_work.len() {
        println!(
            ...
            i,
            employee_longest_nonstop_work[i]
        );
    }
...
fn longest_period(working_slots: Vec<u8>) -> u8 {
    ...
    for slot in &slot_set {
        if !slot_set.contains(&(slot - 1)) {
            ...
            let mut corrent_consecutive_slot = 1;
            while slot_set.contains(&(&current_slot + 1)) {
                current_slot += 1;
                corrent_consecutive_slot += 1;
            }

            if corrent_consecutive_slot > longest_busy_period {
                longest_busy_period = corrent_consecutive_slot;
            }
        }
    }
